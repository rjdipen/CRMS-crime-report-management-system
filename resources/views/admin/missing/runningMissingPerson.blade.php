        ...
        <div class="col-md-12">
            <div class="panel panel-flat border-top-success">
                <div class="panel-heading">
                    <h6 class="panel-title">All Running Missing Person</h6>
                </div>
                    ...
                    <table class="table table-bordered table-condensed table-hover table-striped datatable">
                        ...
                        @foreach($table as $row)
                            <tr>
                                ...
                                <td><a class="btn btn-xs btn-primary p-0" href="{{action('Missing\MissingPersonController@complete', ['id' => $row->missingPersonID])}}"  onclick="return confirm('Are you sure to Approved Post?')" title="End">{{$row->status}}</a></td>
                                    ...
                                    <ul class="icons-list">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-right bg-slate">
                                                ...
                                                <li><button  class="btn bg-slate-600 no-border-radius ediBtn"data-name="{{$row->name}}" data-gender="{{$row->genderID}}"  data-policestation="{{$row->policeStationID}}" data-age="{{$row->age}}"
                                                             data-height="{{$row->height}}"  data-weight="{{$row->weight}}" data-bodycolor="{{$row->bodyColor}}" data-haircolor="{{$row->hairColor}}"  data-division="{{$row->divisionID}}"
                                                             data-clothes="{{$row->clothes}}" data-missingdate="{{$row->missingDate}}"  data-contact="{{$row->contact}}" data-tag="{{$row->tag}}" data-description="{{$row->description}}"  data-id="{{$row->missingPersonID}}" data-toggle="modal" data-target="#ediModal" style="width: 100%;text-align: left;"><i class="icon-pencil7 pr-10"></i>Edit</button></li>
