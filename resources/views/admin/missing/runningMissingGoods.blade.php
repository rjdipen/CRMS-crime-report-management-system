        ...
        <div class="col-md-12">
            <div class="panel panel-flat border-top-success">
                <div class="panel-heading">
                    <h6 class="panel-title">All Running Missing Goods</h6>
                </div>
                    ...
                    <table class="table table-bordered table-condensed table-hover table-striped datatable">
                        ...
                        @foreach($table as $row)
                            <tr>
                                ...
                                <td>{{$row->lPlace}}</td>
                                <td>{{$row->contact}}</td>
                                <td><a class="btn btn-xs btn-primary p-0" href="{{action('Missing\MissingGoodsController@complete', ['id' => $row->missingGoodsID])}}"  onclick="return confirm('Are you sure to Approved Post?')" title="End">{{$row->status}}</a></td>
                                    ...
                                    <ul class="icons-list">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-right bg-slate">
                                                <li>
                                                    <button type="button" class="btn bg-slate-600 no-border-radius ediBtn" data-toggle="modal" data-target="#showModal{{$row->missingGoodsID}}" style="width: 100%;text-align: left;">
                                                        <i class="icon-eye pr-10"></i> View Detail
                                                    </button>
                                                </li>
                                                <li><button  class="btn bg-slate-600 no-border-radius ediBtn"data-name="{{$row->name}}" data-imechessis="{{$row->imeChessis}}"  data-lplace="{{$row->lPlace}}" data-goodscolor="{{$row->goodsColor}}"
                                                             data-missingdate="{{$row->missingDate}}" data-policestation="{{$row->policeStationID}}" data-goodscategory="{{$row->goodsCategoryID}}" data-goodssubcategory="{{$row->goodsSubcategoryID}}" data-contact="{{$row->contact}}" data-tag="{{$row->tag}}" data-description="{{$row->description}}"  data-id="{{$row->missingGoodsID}}" data-toggle="modal" data-target="#ediModal" style="width: 100%;text-align: left;"><i class="icon-pencil7 pr-10"></i>Edit</button></li>
